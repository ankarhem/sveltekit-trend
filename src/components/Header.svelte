<script lang="ts">
	import Cart from 'carbon-icons-svelte/lib/ShoppingCart.svelte';
	import Menu from 'carbon-icons-svelte/lib/Menu.svelte';
	import type { Categories } from '../routes/+layout.svelte';

	const logoSrc =
		'data:image/svg+xml;base64,';

	export let categories: Categories;
</script>

<div
	class="flex justify-center gap-2 bg-secondary/80 py-0.5 text-xs text-secondary-content uppercase"
>
	<span
		>Cyber sale starts now: <a href="/campaigns" class="px-1 underline">Check out our deals</a
		></span
	>
</div>

<header class="grid grid-cols-3 lg:grid-cols-2 bg-base-100 px-4 shadow-md py-1">
	<button class="lg:hidden"><Menu class="box-content h-5 w-5 py-2" /></button>

	<div class="flex items-center">
		<a href="/" class="">
			<img alt="logo" src={logoSrc} />
		</a>
		<nav class="hidden lg:flex items-center px-4">
			<ul class="flex gap-4">
				{#each categories as category}
					{#if category}
						<li>
							<a href={category.primaryRoute?.path} class="block px-1 py-2">
								{category.name}
							</a>
						</li>
					{/if}
				{/each}
			</ul>
		</nav>
	</div>

	<div class="flex items-center justify-end">
		<button>
			<Cart class="box-content h-5 w-5 p-2" />
		</button>
	</div>
</header>
