<script lang="ts">
	import Cart from 'carbon-icons-svelte/lib/ShoppingCart.svelte';
	import Menu from 'carbon-icons-svelte/lib/Menu.svelte';

	const logoSrc =
		'data:image/svg+xml;base64,';
</script>

<div class="flex justify-center gap-2 bg-neutral py-0.5 text-xs text-neutral-content uppercase">
	<span>Cyber sale starts now: <a href="/" class="px-1 underline">Check out or deals</a></span>
</div>

<header class="grid grid-cols-3 bg-base-100 px-4 shadow-md">
	<button class="lg:hidden"><Menu class="box-content h-5 w-5 py-2" /></button>
	<nav class="hidden lg:flex items-center">
		<a href="/" class="block px-1 py-2">Startpage</a>
		<a href="/accessories" class="block px-1 py-2">Accessories</a>
		<a href="/bikes" class="block px-1 py-2">Bikes</a>
		<a href="/does-not-exist" class="block px-1 py-2">404</a>
	</nav>
	<a href="/" class="flex items-center justify-self-center">
		<img alt="logo" src={logoSrc} />
	</a>
	<div class="flex items-center justify-end">
		<button>
			<Cart class="box-content h-5 w-5 p-2" />
		</button>
	</div>
</header>
